...
router.post("/interests", [upload.single(), auth], async (req, res) => {
  ...
  if (_id != "undefined" && _id != null) {
    ...
    if (CategoryResponse) {
      ...
      const SearchCategoryResponse = await CategoryModel.find({
        Catmaster: Catmaster,
      });
...
router.get("/menubar", auth, async (req, res) => {
  ...
  for (let catmaster of CatmasterRespone) {
    let innerItemsArray = [];
    const CategoryRespone = await CategoryModel.find({
      Status: true,
      Catmaster: catmaster._id,
    });
...
router.post("/blogs", [upload.single(), auth], async (req, res) => {
  ...
  if (_id != "undefined" && _id != null) {
    ...
    if (CategoryResponse) {
      const Catmaster = CategoryResponse.Catmaster;
      const SearchBlogResponse = await BlogModel.find({
        Catmaster: Catmaster,
      }).limit(3);
